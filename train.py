...
from nets.yolo import Enhance_yolo
...
if __name__ == "__main__":
    ...
    #   创建yolo模型并且加载预训练模型
    #------------------------------------------------------#
    model_part1 = Enhance_yolo()
    #weights_init(model_part1)

    model_path = 'model_data/best_yolo.pth'
    model_dict      = model_part1.yolo.state_dict() # 读取当前模型参数
    ...
    model_part1.yolo.load_state_dict(model_dict) # 加载模型参数

# 加载增强部分
    model_part2 = Enhance_yolo()
    model_part2_path = 'model_data/enhance_single_new.pth'
    model2_dict = model_part2.yolo.state_dict()
    pre_dict = torch.load(model_part2_path, map_location = device) # 读取预训练模型
    load_key2, no_load_key2, temp_dict2 = [], [], {}
    for k, v in pre_dict.items():
        if k in model2_dict.keys() and np.shape(model2_dict[k]) == np.shape(v):
            temp_dict2[k] = v
            load_key2.append(k)
        else:
            no_load_key2.append(k)
    model2_dict.update(temp_dict2) # 是用预训练的模型更新当前参数
    model_part2.yolo.load_state_dict(model2_dict) # 加载模型参数

    model = Enhance_yolo()
    new_dict = model.yolo.state_dict()
    for k in new_dict.keys():
        if 'inc' in k or 'down1' in k or 'down2' in k or 'down3' in k or 'down4' in k or 'up1' in k or 'up2' in k or 'up3' in k or 'up4' in k or 'outc' in k or 'sigmoid' in k:
            new_dict[k] = model_part2.yolo.state_dict()[k]
        else:
            new_dict[k] = model_part1.yolo.state_dict()[k]
    model.yolo.load_state_dict(new_dict)


    # 显示没有匹配上的key
    print("\nSuccessful Load Key:", str(load_key)[:500], "……\nSuccessful Load Key Num:", len(load_key))
    print("\nFail To Load Key:", str(no_load_key)[:500], "……\nFail To Load Key num:", len(no_load_key))
    print("\nSuccessful Load Key:", str(load_key2)[:500], "……\nSuccessful Load Key Num:", len(load_key2))
    print("\nFail To Load Key:", str(no_load_key2)[:500], "……\nFail To Load Key num:", len(no_load_key2))

    #----------------------#
    #   获得损失函数
    #----------------------#
    yolo_loss = YOLOLoss(anchors, num_classes, input_shape, Cuda, anchors_mask)
    enhance_loss = torch.nn.L1Loss()
    #----------------------#
    #   记录Loss
    #----------------------#
    if local_rank == 0:
    ...
    if True:
        ...
        for k, v in model.named_modules():
            if hasattr(v, "bias") and isinstance(v.bias, nn.Parameter): # bias
                pg2.append(v.bias)    
            if isinstance(v, nn.BatchNorm2d) or "bn" in k:              # 有归一化操作
                pg0.append(v.weight)    
            elif hasattr(v, "weight") and isinstance(v.weight, nn.Parameter): # weight
                pg1.append(v.weight)   
        optimizer = {
            'adam'  : optim.Adam(pg0, Init_lr_fit, betas = (momentum, 0.999)),
            'sgd'   : optim.SGD(filter(lambda p: p.requires_grad, pg0), Init_lr_fit, momentum = momentum, nesterov=True)
        }[optimizer_type]
        optimizer.add_param_group({"params": filter(lambda p: p.requires_grad, pg1) , "weight_decay": weight_decay})
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            for k, v in model.yolo.named_modules(): 
                if 'inc' in k or 'down1' in k or 'down2' in k or 'down3' in k or 'down4' in k or 'up1' in k or 'up2' in k or 'up3' in k or 'up4' in k or 'outc' in k or 'sigmoid' in k:    
                    v.requires_grad = flag
                else:
                    v.requires_grad = not flag 

            fit_one_epoch(model_train, model, enhance_loss, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir, local_rank)
